...
void TinyGLRenderer::setViewport(const Common::Rect &rect) {
	...
	// test scissoring with half the viewport dimensions (original implementation will use entire viewport dimensions)
	tglScissor(rect.left, g_system->getHeight() - rect.bottom, rect.width() / 2, rect.height() / 2);
}
